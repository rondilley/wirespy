...
void *xmemcpy_(void *d_ptr, const void *s_ptr, const size_t size, const char *filename,
  ...
  if (dest_size > 0) {
    if (dest_size < size) {
      ...
      fprintf(stderr,
              "memcpy called with size (%lu) larger than dest buffer 0x%08x "
              "(%lu) at %s:%d\n",
    ...
    if (source_size > 0) {
      if (source_size < size) {
        ...
        fprintf(stderr,
                "memcpy called with size (%lu) larger than source buffer 0x%08x "
                "(%lu) at %s:%d\n",
...
void *xmemset_(void *ptr, const char value, const size_t size,
...
#ifdef DEBUG_MEM
  /* XXX need to add code to test if set area is a known buffer */
#endif
...
int xmemcmp_(const void *s1, const void *s2, size_t n, const char *filename,
  ...
  /* XXX confirm that the compare buffers are known */
#ifdef DEBUG_MEM
  fprintf(stderr, "0x%x memcmp against 0x%x %s:%d (%lu bytes)\n", s1, s2,
...
void *xrealloc_(void *ptr, size_t size, const char *filename,
                const int linenumber) {
  void *result;
#ifdef MEM_DEBUG
  PRIVATE struct Mem_s *d_result;
...
#ifdef DEBUG_MEM
  fprintf(stderr, "0x%x realloc %s:%d (%lu bytes)\n", result, filename,
          linenumber, size);
#endif
...
void xfree_(void *ptr, const char *filename, const int linenumber) {
  ...
  if (!found) {
    fprintf(stderr,
            "free() called with 0x%08x ptr but not found in debug object list at "
            "%s:%d\n",
...
char *xstrcpy_(char *d_ptr, const char *s_ptr, const char *filename,
  ...
  if (dest_size > 0) {
    if (dest_size < size) {
      /* attempting to copy too much data into dest */
      fprintf(stderr,
              "strcpy called with size (%lu) larger than dest buffer 0x%08x "
              "(%lu) at %s:%d\n",
              size, d_ptr, dest_size, filename, linenumber);
      XFREE_ALL();
      exit(EXIT_FAILURE);
    ...
    if (source_size > 0) {
      if (source_size < size) {
        /* attempting to copy too much data from source */
        fprintf(stderr,
                "strcpy called with size (%lu) larger than source buffer 0x%08x "
                "(%lu) at %s:%d\n",
                size, s_ptr, source_size, filename, linenumber);
        XFREE_ALL();
        exit(EXIT_FAILURE);
